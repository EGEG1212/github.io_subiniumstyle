...
## 🤔제목(Hearder)
`h1`부터 `h6`까지 표현할 수 있으며, `#`의 개수로 표현 가능합니다.
==, --, -------- 로도  표현 가능합니다.
✍작성코드(.md)
```
# H1 태그
...
---
## 🤔수평선
`*`이나 `-`, `_`등을 3개 이상 입력하면 구분선, 수평선이 됩니다.
띄어쓰기를 중간에 삽입해도 가능합니다.
...
#### 1. 순서가 없는 리스트
특징을 나열할 때 사용합니다.
`-`, `*`, `+` 을 사용하여 ✨한칸 띄어서 작성합니다.(모두 동일하게 적용)
...
---
## 🤔코드 블럭(코드 여러 줄) & 하이라이트
``백틱( ` )``을 3개 사용하여 코드블럭을 작성합니다.
첫번째줄 백틱3개 옆에 원하는 문법을 입력하면 색을 입혀줍니다.
예) python cpp sh쉘 javascript
...
자동중앙정렬 와이드100%로 삽입됩니다.
✨사이즈를 조절하고 싶다면 img태그 `<img src="" width=1000>`로 작성합니다.
...
---
## 🤔테이블(Table)
`|`로 셀을 구분하며 폰트의 기본적인 스타일이 모두 적용 가능합니다.
`-`하이픈으로 구분된 셀 왼쪽, 양쪽, 오른쪽에 `:`콜론을 붙일 경우
...
---
## 🤔체크박스(Check Box)
`-`, `*`, `+` 뒤에 띄어쓰기 후 `[ ]`대괄호를 넣어 또 띄어쓰기 후 작성합니다.
...
---
# 마치며...
짧게, 부분적으로 알고 있던 마크다운을 한곳에 모아 보기 편하게 정리했습니다.
역시나 내가 보기 편하게 :-) 


---
참고: T아카데미- 안수빈 <https://tacademy.skplanet.com/live/player/onlineLectureDetail.action?seq=171>
참고: T아카데미- 이수진 <https://tacademy.skplanet.com/live/player/onlineLectureDetail.action?seq=130>
참고: <http://nblog.syszone.co.kr/archives/9917>